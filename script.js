const STORE_KEY = 'btrtAwardsRealMockV5';
...
function renderReviewList() {
  ...
  const nominations = state.nominations.filter((nom) => {
    const haystack = [nom.id, nom.nomineeName, nom.nomineeCompany, nom.nomineeDepartment, nom.values?.join(' '), nom.status].join(' ').toLowerCase();
...
function renderReviewCard(nom) {
  ...
  return `
    <article class="nom-card" data-id="${nom.id}">
      <div class="nom-head">
        <div>
          <h3>${escapeHtml(nom.nomineeName)}</h3>
          <div class="id-line">${nom.id} • Submitted ${escapeHtml(nom.submittedAt)} by ${escapeHtml(nom.nominatorName)} (${escapeHtml(nom.relationship)})</div>
        </div>
      ...
      <div class="meta-grid">
        ...
        <div><span>Evidence</span><strong>${escapeHtml(nom.evidenceStrength || '-')}</strong></div>
      </div>
      <div class="star-box">
...
function renderJudgeList() {
  const state = getState();
  const finalists = state.nominations.filter((nom) => nom.status === 'Finalist');
  if (!finalists.length) {
...
function renderJudgeCard(nom) {
  const judgeName = ($('#judgeName')?.value || 'Judge 01').trim();
  const existing = getState().scores?.[nom.id]?.[judgeName] || { ratings: {} };
  ...
  return `
    <article class="nom-card" data-id="${nom.id}">
      <div class="nom-head">
        <div>
          <h3>${escapeHtml(nom.nomineeName)}</h3>
          <div class="id-line">${nom.id} • ${escapeHtml(nom.nomineeCompany)} • ${escapeHtml(nom.nomineeDepartment)}</div>
        </div>
        <span class="badge finalist">Finalist</span>
      </div>
      <div class="meta-grid">
        <div><span>Values</span><strong>${escapeHtml((nom.values || []).join(', '))}</strong></div>
        <div><span>Impact Scope</span><strong>${escapeHtml(nom.impactScope)}</strong></div>
        <div><span>Director Approval</span><strong>${escapeHtml(nom.directorApproval)}</strong></div>
        <div><span>Company Score</span><strong>${weightedTotal(nom.review?.ratings || {}).toFixed(1)} / 100</strong></div>
      </div>
      <div class="star-box">
      ...
      <label style="margin-top:12px;">Judge comments
        <textarea data-score-comment placeholder="Optional judging notes">${escapeHtml(existing.comment || '')}</textarea>
      </label>
      <div class="actions" style="margin-top:12px;">
        <button class="btn primary" data-action="save-score">Save Judge Score</button>
      </div>
    </article>`;
}
...
function saveJudgeScore(card) {
  ...
  state.scores[card.dataset.id][judgeName] = {
    ratings,
  ...
  saveState(state);
  toast(`Score saved for ${judgeName}.`);
}
...
function renderRankingTable() {
  const state = getState();
  const finalists = state.nominations
    .filter((nom) => nom.status === 'Finalist')
    .map((nom) => ({ ...nom, score: getAverageScore(nom.id) }))
    .sort((a, b) => b.score.average - a.score.average);
  const tbody = $('#rankingTable tbody');
  tbody.innerHTML = finalists.map((nom, index) => `
    <tr>
      <td>${index + 1}</td>
      <td><strong>${escapeHtml(nom.nomineeName)}</strong><br><span class="muted">${nom.id}</span></td>
      <td>${escapeHtml(nom.nomineeCompany)}</td>
...
function renderReports() {
  ...
  const finalists = state.nominations.filter((n) => n.status === 'Finalist').map((n) => ({
    id: n.id,
  ...
  $('#adminPreview').textContent = JSON.stringify({
    ...
    note: 'Static GitHub Pages mock. Data is stored in this browser only.',
...
function exportJson() {
  downloadFile('btrt-awards-mock-export.json', JSON.stringify(getState(), null, 2), 'application/json');
}

function exportCsv() {
  const rows = [['ID', 'Status', 'Nominee', 'Company', 'Department', 'Values', 'Nominator', 'Nominator Email', 'Director Approval', 'Company Score', 'Judge Average', 'Judge Count']];
  ...
  downloadFile('btrt-awards-mock-export.csv', csv, 'text/csv');
}
...
async function copyJudgePack() {
  const state = getState();
  const finalists = state.nominations.filter((n) => n.status === 'Finalist');
  const text = finalists.map((n, i) => {
    const avg = getAverageScore(n.id);
    return `${i + 1}. ${n.nomineeName} — ${n.nomineeCompany}\nValues: ${(n.values || []).join(', ')}\nSTAR Result: ${n.result}\nCompany Score: ${weightedTotal(n.review?.ratings || {}).toFixed(1)}/100\nJudge Average: ${avg.average ? avg.average.toFixed(1) : '-'} (${avg.count} score records)\n`;
  }).join('\n');
  await navigator.clipboard.writeText(text || 'No finalists yet.');
  toast('Finalist Judge Pack copied.');
}
...
function seedDemoData() {
  const demo = [
    {
      ...
      nomineeName: 'Nadia Lim', nomineeEmail: 'nadia.lim@example.com', nomineeCompany: 'YTL Hotels', nomineeDepartment: 'Guest Experience', jobTitle: 'Assistant Manager', location: 'Kuala Lumpur, Malaysia',
      ...
      task: 'Nadia had to coordinate responses across hotel operations, brand, CRM and frontline teams while protecting the guest experience.',
      action: 'She created a same-day escalation tracker, aligned hotel teams on the response script and personally followed up with affected teams until cases were resolved.',
      result: 'The team closed all priority cases within 48 hours and used her tracker as the template for future peak-period guest recovery.',
      impactScope: 'Company / Business Unit', evidenceStrength: 'Strong measurable impact', evidenceNotes: 'Guest recovery tracker, CRM notes, thank-you email from operations.',
    ...
    },
    {
      ...
      nomineeName: 'Farah Aziz', nomineeEmail: 'farah.aziz@example.com', nomineeCompany: 'YTL Cement', nomineeDepartment: 'Operations', jobTitle: 'Process Engineer', location: 'Perak, Malaysia',
      ...
      task: 'Farah was asked to investigate the root cause and propose a practical fix without disrupting daily operations.',
      action: 'She analysed shift logs, mapped the downtime triggers and trialled a revised maintenance checklist with supervisors over two weeks.',
      ...
      impactScope: 'Department', evidenceStrength: 'Exceptional business and people impact', evidenceNotes: 'Pilot downtime report and supervisor feedback.',
    ...
    },
    {
      ...
      nomineeName: 'Marcus Tan', nomineeEmail: 'marcus.tan@example.com', nomineeCompany: 'YTL PowerSeraya', nomineeDepartment: 'Digital', jobTitle: 'Senior Analyst', location: 'Singapore',
      values: ['Honesty', 'Hard Work'], situation: 'A reporting dashboard had inconsistent figures before a leadership update.',
      task: 'Marcus needed to find the source of the discrepancy and rebuild confidence in the numbers.',
      action: 'He traced the error to duplicate source rows, documented the issue openly and rebuilt the refresh checklist.',
      ...
      impactScope: 'Company / Business Unit', evidenceStrength: 'Strong measurable impact', evidenceNotes: 'Dashboard change log.',
    ...
    },
    {
      ...
      nomineeName: 'Priya Raman', nomineeEmail: 'priya.raman@example.com', nomineeCompany: 'Wessex Water', nomineeDepartment: 'Customer Support', jobTitle: 'Team Lead', location: 'Bath, United Kingdom',
      ...
      task: 'Priya had to stabilise service levels and support new joiners at the same time.',
      action: 'She split the backlog by urgency, paired new joiners with experienced agents and introduced a daily 15-minute triage huddle.',
      result: 'The backlog was cleared within the month and team escalation quality improved according to QA checks.',
      impactScope: 'Team', evidenceStrength: 'Clear outcome, some evidence', evidenceNotes: 'QA checks and backlog report.',
    ...
    },
    {
      ...
      nomineeName: 'Ivan Koh', nomineeEmail: 'ivan.koh@example.com', nomineeCompany: 'YTL Land & Development', nomineeDepartment: 'Sales Gallery', jobTitle: 'Executive', location: 'Kuala Lumpur, Malaysia',
      values: ['Vitality'], situation: 'A weekend launch event had unexpectedly high footfall.',
      task: 'Ivan helped manage visitor flow and support the sales gallery team.',
      action: 'He stayed late, redirected guests and helped compile follow-up leads.',
      result: 'The team completed the event without major delays, but supporting figures are still being collected.',
      impactScope: 'Team', evidenceStrength: 'Needs more detail', evidenceNotes: 'Awaiting lead report.',
    ...
    },
    {
      ...
      nomineeName: 'Arif Rahman', nomineeEmail: 'arif.rahman@example.com', nomineeCompany: 'YTL Communications / YES 5G', nomineeDepartment: 'Network Experience', jobTitle: 'Manager', location: 'Kuala Lumpur, Malaysia',
      ...
      task: 'Arif had to make customer pain points visible and turn them into prioritised fixes.',
      action: 'He built a simple issue heatmap, coordinated daily stand-ups and closed the loop with customer-facing teams.',
      result: 'The sprint resolved the highest-frequency issues and reduced repeated escalations for the pilot area.',
      impactScope: 'Group-wide', evidenceStrength: 'Exceptional business and people impact', evidenceNotes: 'Issue heatmap, escalation tracker and pilot report.',
    ...
    },
    {
      ...
      nomineeName: 'Liew Kai Ming', nomineeEmail: 'kaiming.liew@example.com', nomineeCompany: 'YTL Foundation', nomineeDepartment: 'Programmes', jobTitle: 'Programme Coordinator', location: 'Kuala Lumpur, Malaysia',
      ...
      task: 'Kai Ming supported partner coordination and attendance tracking.',
      action: 'He consolidated contact lists, clarified responsibilities and prepared reminders for volunteers.',
      result: 'Attendance improved and the programme team reported smoother on-ground coordination.',
      impactScope: 'Customer / Community', evidenceStrength: 'Clear outcome, some evidence', evidenceNotes: 'Volunteer schedule and attendance list.',
    ...
    },
    {
      ...
      nomineeName: 'Grace Wong', nomineeEmail: 'grace.wong@example.com', nomineeCompany: 'YTL Data Center', nomineeDepartment: 'Admin', jobTitle: 'Administrator', location: 'Johor, Malaysia',
      values: ['Honesty'], situation: 'Grace helped correct a vendor contact list.',
      task: 'She needed to update the internal contact document.',
      action: 'She checked the latest details and informed the team.',
      result: 'The list was corrected.',
      impactScope: 'Team', evidenceStrength: 'Needs more detail', evidenceNotes: 'Contact list update.',
  ...
  state.scores = {
    'BTRT-DEMO1': {
      'Judge 01': { ratings: { values: 5, impact: 4, scale: 4, difficulty: 4, story: 5 }, total: 45, comment: 'Excellent cross-functional story.', scoredAt: new Date().toISOString() },
      'Judge 02': { ratings: { values: 5, impact: 4, scale: 3, difficulty: 4, story: 5 }, total: 43, comment: 'Strong values and process impact.', scoredAt: new Date().toISOString() }
    },
    'BTRT-DEMO2': {
      'Judge 01': { ratings: { values: 5, impact: 5, scale: 3, difficulty: 5, story: 4 }, total: 47, comment: 'Best measurable operations impact.', scoredAt: new Date().toISOString() },
      'Judge 02': { ratings: { values: 4, impact: 5, scale: 3, difficulty: 5, story: 4 }, total: 39, comment: 'Strong result, slightly narrower influence.', scoredAt: new Date().toISOString() }
    },
    'BTRT-DEMO6': {
      'Judge 01': { ratings: { values: 5, impact: 5, scale: 5, difficulty: 4, story: 5 }, total: 49, comment: 'Strongest group-wide relevance.', scoredAt: new Date().toISOString() }
    }
  };
  saveState(state);
...
function fillNominationSample() {
  ...
  toast('Sample nomination filled.');
}
...
function submitNomination(event) {
  ...
  saveState(state);
  form.reset();
  toast('Nomination submitted. Reviewer can now see it in the workspace.');
}
...
function init() {
  ...
  $('#clearData').addEventListener('click', () => {
    if (!confirm('Clear all mock nominations and scores in this browser?')) return;
  ...
  $('#judgeName').addEventListener('input', renderJudgeList);
  ...
  if (!$('#judgeName').value) $('#judgeName').value = 'Judge 01';
  renderAll();
}
